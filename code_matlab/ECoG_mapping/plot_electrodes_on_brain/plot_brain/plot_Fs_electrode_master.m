function S_brain = plot_Fs_electrode_master(subjid,H,S_brain,SUMAsrf,elocDir,ch_labels,eSize,eColor,textFlag,shading,order)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%
% subjid    : subject 
% H         : handle of graphic object
% SUMAsrf   : surface type, 'inflated', 'pial', 'flat'
% elocDir   : path of repository containing electrodes 
% ch_labels : channels labels 
% eColor    : electrode color 
% eSize     : Radius in mm (What radius?)
% textFlag  : ?????????????????????????????
% shading   : ?????????????????????????????
% order     : ?????????????????????????????
...
if nargin < 7   eColor = [0 0 0] ;   end
if nargin < 8 textFlag = 1 ;         end 
if nargin < 9  shading = 'gouraud' ; end
if nargin < 10 order   = 'top' ;     end

% Set manually whether you want to use projected electrode locations:
projectedFlag = 0;

if isempty(H)||isempty(S_brain) error('Missing arguments!'); end
...
for i = 1:numel(ch_labels)
    ...
    for h=axesHandle'
        ...
        if strcmpi(S_brain.plotsurf,'flat')
