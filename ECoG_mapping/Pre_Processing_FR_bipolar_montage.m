...
for initials = subjects(end)
    ...
    for run=['1' '2']
        ...
        EEG = pop_select(EEG,'nochannel',find(rm_idx)); %remove chan
        ...
        EEG=ReadElectrodeCoord(EEG,channel_location_file,maindir); %NEED helper function !!!!!
        [ALLEEG, EEG, CURRENTSET] = eeg_store(ALLEEG, EEG, CURRENTSET);
        eeglab redraw;
...
%         S_brain=plot_Fs_electrode(initials,H_brain,S_brain,SUMAsrf,elocDir,ch_labels,eColor,[],eSize,textFlag,'gouraud','top');
        ...
        for i=1:numel(good_ch_labels)
            XYZ(i,:) = electrodes.coord.afniXYZ(strcmpi(electrodes.elecNames,good_ch_labels{i}),:); %electrodes structure coming from ????
        end        
