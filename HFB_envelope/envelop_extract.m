...
%-Try different model order and spectral radius
%% Data input
tsdim=6;
varmorder = 5;
nobs = 30000;
...
apsd_tsdata_fname=['auto_cpsd_tsdata',num2str(morder),'_''tsdim', num2str(tsdim),'_','specrad',num2str(specrad),'.png'];
...
[cpsd_envelope,f,fres] = tsdata_to_cpsd(tsdata,false,fs,[],[],fres,true,false);
apsd_envelope_figure_path=[pwd,'\figures\Hilbert_envelope\'];
apsd_envelope_fname=['auto_cpsd_envelope',num2str(morder),'_''tsdim', num2str(tsdim),'_','specrad',num2str(specrad),'.png'];
...
tgc_fname=['temporal_gc_',num2str(fcut_low),'-',num2str(fcut_high),'Hz_','env_morder',num2str(env_morder),'_','morder',num2str(morder),'_''tsdim', num2str(tsdim),'_','specrad',num2str(specrad),'.png'];
...
if isnumeric(plotm), plotm = plotm+1; end
plot_sgc({spwcgc_tsdata,spwcgc_envelope},freqs,'Spectral Granger causalities (blue = tsdata, red = envelope)',plotm);
ylim([0 0.25])
xlim([50 100])
sgc_figure_path=[pwd,'\figures\Hilbert_envelope\'];
sgc_fname=['spectral_gc_',num2str(fcut_low),'-',num2str(fcut_high),'Hz_','env_morder',num2str(env_morder),'_','morder',num2str(morder),'_''tsdim', num2str(tsdim),'_','specrad',num2str(specrad),'.png'];
