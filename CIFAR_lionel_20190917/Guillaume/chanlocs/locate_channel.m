%% Writes ROI and label channels in the EEG structure
...
chan_name=chan_name';
%create array of channels region according to EEG.urnumber
chan_region=chan2region(chan_name,elec_name,elec_region);
%Writes channels in the EEG structure
for i=1:size(EEG.chanlocs,2)
    EEG.chanlocs(i).region=chan_region(i);
end
%Write regions of interest to the structure
...
%Write ROI index to each channel region in the EEG structure
