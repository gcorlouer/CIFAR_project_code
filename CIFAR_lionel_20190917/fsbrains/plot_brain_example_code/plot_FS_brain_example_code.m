...
fsdatadir = fullfile(datadir,'fsbrains','plot_brain_example_code','default_freesrufer_surface');
fsavgedir = fullfile(datadir,'fsbrains','fsaverage');
...
if ~isfield(S_brain,'meshdir'); S_brain.meshdir=fsavgedir; end
...
if ismember(S_brain.plotsurf,{'flat', 'inflated'})
    ...
    load(fullfile(fsdatadir,'SUMA_default.mat'));
else
    %load('D:\plot_brain\default_freesrufer_surface\SUMA_pial_blank.mat');
    load(fullfile(fsdatadir,'SUMA_pial_blank.mat'));
...
%{
    ...
    mex(fullfile(fsdatadir,'smoothpatch','smoothpatch_curvature_double.c'),'-v')
    mex(fullfile(fsdatadir,'smoothpatch','smoothpatch_inversedistance_double.c'),'-v')
    mex(fullfile(fsdatadir,'smoothpatch','vertex_neighbours_double.c'),'-v')
%}
