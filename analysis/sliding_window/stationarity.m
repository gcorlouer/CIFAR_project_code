...
%problem : When BP is on often cholesky decomposition fails
...
tail_fig_filename=[filename,'_BP_',num2str(BP),'_ROI_',num2str(-schans),'_wind_',num2str(wind(1)),'s.fig'];
close all
%% Slide cpsd
s=sliding_cpsd(X,ts)
...
fig_filename=['ss_model_order_',filename,'_BP_',num2str(BP),'_wind_',num2str(wind(1)),'s.fig'];
% fig_title=[fig_title,', VAR model order estimation along ',num2str(wind(1)),'s',' sliding window '];
% title(fig_title)
saveas(gcf,[fig_filepath,fig_filename]);
close all
...
fig_filename=['SS_model_specrad_',filename,'_BP_',num2str(BP),'_wind_',num2str(wind(1)),'s.fig'];
% fig_title=[fig_title,', VAR model order estimation along ',num2str(wind(1)),'s',' sliding window '];
% title(fig_title)
saveas(gcf,[fig_filepath,fig_filename]);
close all