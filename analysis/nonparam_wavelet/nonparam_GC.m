...
if ~exist('tseg',      'var'),       tseg           = [10 15];                                          end % start/end times (empty for entire time series)
...
if ~exist('freq',   'var'),          freq           = band_low:band_low + nband*band_size ;             end
if ~exist('omega0',   'var'),        omega0         = 6 ;                                               end
if ~exist('pad0',   'var'),          pad0           = 1 ;                                               end
if ~exist('flgCond',   'var'),       flgCond        = 1 ;                                               end
%if ~exist('fRes',   'var'),          fRes           = 1024 ;                                            end
if ~exist('NW',   'var'),            NW             = 4 ;                                               end
if ~exist('m',   'var'),             m              = 4 ;                                               end
if ~exist('wavelet',   'var'),       wavelet        = 'mtap' ;                                          end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Import data
...
%Name tail of figure file to save
...
%% Reshape data for compatibility with nonparamGC toolbox

X_wvlt        = zeros(size(X,2),1,size(X,1));
X_wvlt(:,1,:) = X';
X             = X_wvlt;
Nt=size(X,1);
%% Non parametric GC analysis

if     wavelet == 'Morl'
    [freq, causality, COH, iCOH, power, S] = compute_nonparGGC_wavelet_Morlet(X, fs,freq, pad0, flgCond, omega0);
elseif wavelet == 'mtap'
    fRes=fs/nobs; 
    [freq, causality, COH, iCOH, power, S] = compute_nonparGGC_multitaper(X, fs, fRes, freq, flgCond, NW);
else
    [freq, causality, COH, iCOH, power, S] = compute_nonparGGC_wavelet_Paul(X, fs, freq, pad0, flgCond, m)
end